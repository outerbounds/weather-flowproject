...
from metaflow import step, trigger_on_finish, current, project, Config, config_expr, card, Parameter, pypi, resources
...
class ForecastFlow(BaseFlow):
    ...
    qa_zipcode = Parameter('qa_zipcode', default='02108', help='visualize this zipcode for sanity check')

    @snowflake
    @step
    def start(self):
        self.timestamp = self.sensor_value
        self.since = datetime.fromisoformat(self.timestamp[:-1]) - timedelta(days=self.window)
        print("Querying data since", self.since)
        self.data = self.query_snowflake(template=('historical_temperature', [self.since]))
        self.next(self.forecast)
    
    @card(type='blank', refresh_interval=1)
    @pypi(python='3.11', packages={'pandas': '2.2.3', 'sktime': '0.35.0', 'statsmodels': '0.14.4'})
    ...
    def end(self):
        if self.qa_zipcode in self.forecasts:
            ...
            current.card.append(Markdown("**Blue** is historical data, **orange** is our forecast"))
